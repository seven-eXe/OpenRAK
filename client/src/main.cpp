...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	SYSTEMTIME time;
	GetLocalTime(&time);
	Log("[OPENRAK] Запущен. Версия: " RAKSAMP_VERSION);
	Log("[OPENRAK] Работает на основе RakSAMP. Авторы: " AUTHOR);
	Log("[OPENRAK] Автор: " OPENRAK_AUTHOR);

	//Log("[OPENRAK] Загрузка Lua скриптов...");

	const auto req_ext = ".lua";
	for (const auto& filename : std::experimental::filesystem::directory_iterator("scripts")) {
		if (!std::experimental::filesystem::path(filename).extension().compare(req_ext))
			LoadScript(filename.path().string().c_str(), false);
	}
	...
	while(1)
	{
		...
		if (iAreWeConnected && iGameInited)
		{
			...
			if(dwLastInfoUpdate && dwLastInfoUpdate < (GetTickCount() - 1000))
			{
				...
				sprintf_s(szInfo, 400, "Сервер: %s\nИгроки: %d\nПинг бота: %d\nХП: %s\nБроня: %s\nСкин: %d\n\n[Позиция]\nX: %.4f     Y: %.4f     Z: %.4f",
				g_szHostName, getPlayerCount(), playerInfo[g_myPlayerID].dwPing, szHealthText, szArmourText, iLocalPlayerSkin, settings.fNormalModePos[0], settings.fNormalModePos[1], settings.fNormalModePos[2]);
...
void Log(char *fmt, ...)
{
	...
	else
	{
		...
		wsprintf(tbuf, "[%02d:%02d:%02d] %s", time.wHour, time.wMinute, time.wSecond, buffer);
