...
#define OPENRAK_INFORMATION     103
#define OPENRAK_INFOTEXT		104
	...
	case WM_CREATE:
		{
			...
			/*hWndChild = CreateWindowEx(0, WC_STATIC, TEXT(""),
			...
			loghwnd = hWndChild;
			SendMessage(loghwnd, LB_SETITEMHEIGHT, 0, 17);
			...
			hWndChild = CreateWindowEx(WS_EX_TRANSPARENT, WC_BUTTONA, "Информация",
				WS_CHILD | WS_VISIBLE | BS_GROUPBOX,
				0, 0, 0, 0, hWnd, (HMENU)OPENRAK_INFORMATION, g_hInst, NULL);
			SendMessage(hWndChild, WM_SETFONT, (WPARAM)g_hfText, FALSE);

			hWndChild = CreateWindowEx(0, WC_STATIC, TEXT("Загрузка..."),
				SS_LEFT | WS_CHILD | WS_VISIBLE,
				0, 0, 0, 0, hWnd, (HMENU)OPENRAK_INFOTEXT, g_hInst, NULL);
			SendMessage(hWndChild, WM_SETFONT, (WPARAM)g_hfText, FALSE);
			texthwnd = hWndChild;
	...
	case WM_DRAWITEM:
		{
			...
			if(lpDrawItem->CtlID == IDC_LSTCUSTOM)
			{
				...
				if (strstr(lpText, "[OPENRAK]"))
					textColor = RGB(0, 0, 170);

				if (strstr(lpText, "[SERVER]"))
					textColor = RGB(0, 170, 0);
				...
				ExtTextOut(lpDrawItem->hDC,
					lpDrawItem->rcItem.left + 5, lpDrawItem->rcItem.top,
	...
	case WM_WINDOWPOSCHANGED:
		{
			...
			if(hDWP = BeginDeferWindowPos(4))
			{
				...
				hDWP = DeferWindowPos(hDWP, GetDlgItem(hWnd, IDC_LSTCUSTOM), NULL,
					10, 10, rc.right - 300, rc.bottom - rcText.bottom - 75,
					SWP_NOZORDER | SWP_NOREDRAW);

				// Information box
				hDWP = DeferWindowPos(hDWP, GetDlgItem(hWnd, OPENRAK_INFORMATION), NULL,
					(rc.right) - 270, 4, 250, 250,
					SWP_NOZORDER | SWP_NOREDRAW);

				//Info text
				hDWP = DeferWindowPos(hDWP, GetDlgItem(hWnd, OPENRAK_INFOTEXT), NULL,
					(rc.right) - 250, 20, 200, 200,
					SWP_NOZORDER | SWP_NOREDRAW);

				// Move/Size the input box
				hDWP = DeferWindowPos(hDWP, GetDlgItem(hWnd, IDC_INPUTBOX), NULL,
					10, rc.bottom - 40, rc.right - 90, 20,
					SWP_NOZORDER | SWP_NOREDRAW);
...
DWORD WINAPI windowThread(PVOID)
{
	...
	g_hfText = CreateFont(-MulDiv(8, GetDeviceCaps(hdcScreen, LOGPIXELSY), 72), // 11pt
	...
	g_hfInfo = CreateFont(-MulDiv(8, GetDeviceCaps(hdcScreen, LOGPIXELSY), 85),
		0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, RUSSIAN_CHARSET, OUT_TT_PRECIS,
		CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, FF_DONTCARE, TEXT("Tahoma"));
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, wcex.lpszClassName, TEXT("OpenRak " RAKSAMP_VERSION),
		WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, CW_USEDEFAULT, CW_USEDEFAULT, 800, 570,
